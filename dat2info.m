...
if isfield(data.ops, 'db_orig')
    ...
    expDate = data.ops.db_orig.date;
    experiments = data.ops.db_orig.expts;
else
...
if iscell(experiments)
    nExps = sum(cellfun(@length, experiments));
else
...
if iscell(animal)
    k = 1;
    for iSet = 1:length(animal)
        for iExp = 1:length(experiments{iSet})
            ExpRefs{k} = dat.constructExpRef(animal{iSet}, expDate{iSet}, ...
                experiments{iSet}(iExp));
            expExists(k) = dat.expExists(ExpRefs{k});
            k = k + 1;
        end
    end
else
...
for iExp = 1:nExps
    ...
    info.F = double(data.Fcell{iExp}(roiIdx, :)');
    info.registeredMIP = data.ops.mimg1(data.ops.yrange(:), data.ops.xrange(:));
    
    info.badFrames = find(data.ops.badframes(frameIdx));
