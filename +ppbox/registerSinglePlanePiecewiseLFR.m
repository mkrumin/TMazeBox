function infoReg = registerSinglePlanePiecewiseLFR(info, options)
...
if isfield(options, 'cropPosition') && isfield(options, 'targetFrameCrop')
    ...
    for iRect = 1:nRects
       ...
       [dx, dy] = img.regTranslationsMKquick(targetBank{iRect}, options.targetFrame(p.ymin:p.ymax, p.xmin:p.xmax), 'noparallel');
        ...
        collage(posDest(iRect).ymin:posDest(iRect).ymax, posDest(iRect).xmin:posDest(iRect).xmax) = targetBank{iRect};
    end
