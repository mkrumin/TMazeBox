...
%     plot(cumsum([pos(1) pos(3)]), cumsum([pos(2) pos(4)]), '.r')
    p.xmin = max(1, round(pos(1)));
    p.xmax = min(size(options.targetFrame, 2), round(pos(1)+pos(3)));
    p.ymin = max(1, round(pos(2)));
    p.ymax = min(size(options.targetFrame, 1), round(pos(2)+pos(4)));
    plot([p.xmin, p.xmin, p.xmax, p.xmax, p.xmin], [p.ymin, p.ymax, p.ymax, p.ymin, p.ymin], 'r:');
    yInd = p.ymin:p.ymax;
    xInd = p.xmin:p.xmax;
    drawnow;
    
    options.targetFrameCropped = selectTargetDev(1, data(yInd, xInd, :), nFrames2Use, floor(nFrames2Use/20), 1, hGauss);
    clear data;
    ...
    colormap gray
    subplot(1, 2, 2);
    pic = options.targetFrame;
    pic(yInd, xInd) = options.targetFrameCropped;
    imagesc(pic);
    axis equal tight
    drawnow
...
for iChunk = 1:nChunks
    ...
    if options.nParThreads == 1
        if options.quickReg
            [dx, dy] = img.regTranslationsMKquick(single(planeDataCropped), ...
                options.targetFrameCropped, 'noparallel');
        else
            [dx, dy] = img.regTranslationsMK(single(planeDataCropped), ...
                options.targetFrameCropped, 'noparallel');
        end
    else
        if options.quickReg
            [dx, dy] = img.regTranslationsMKquick(single(planeDataCropped), ...
                options.targetFrameCropped);
        else
            [dx, dy] = img.regTranslationsMK(single(planeDataCropped), ...
                options.targetFrameCropped);
        end
